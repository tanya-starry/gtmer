...
import { Bot, BarChart3, PenTool, Palette, Globe, Code, Mail, Megaphone } from "lucide-react";
            ...
            >
              ...
              <div className="flex gap-2 pl-2">
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-1.5 h-9 text-xs font-medium rounded-xl transition-all duration-200 shadow-sm ${agent.status === "运行中" ? "text-[#64748B] bg-[#F1F5F9] hover:bg-[#E2E8F0]" : "text-white bg-[#1E40AF] hover:bg-[#2563EB] hover:shadow-md hover:shadow-blue-500/20"}`}
                >
                  {agent.status === "运行中" ? <><Pause className="w-3.5 h-3.5" />暂停</> : <><Play className="w-3.5 h-3.5" />运行</>}
                </button>
              </div>
            </motion.div>
          ))}
            ...
            >
              <div className="sticky top-0 bg-white border-b border-[#E2E8F0] px-6 py-4 flex items-center justify-between z-10">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full flex items-center justify-center text-lg" style={{ backgroundColor: `${selectedAgent.color}10` }}>
                    {selectedAgent.avatar}
                  </div>
                  <div>
                    <h3 className="text-base font-semibold text-[#0F172A]">{selectedAgent.name}</h3>
                    <div className="flex items-center gap-1.5">
                      <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: selectedAgent.status === "运行中" ? "#10B981" : "#94A3B8" }} />
                      <span className="text-xs text-[#64748B]">{selectedAgent.status}</span>
                    </div>
                  </div>
                </div>
                <button onClick={() => setSelectedAgent(null)} className="p-2 text-[#94A3B8] hover:text-[#0F172A] hover:bg-[#F1F5F9] rounded-lg transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="p-6 space-y-6">
                <div>
                  ...
                  <p className="mt-1.5 text-sm text-[#0F172A]">{selectedAgent.specialty}</p>
                </div>

                <div>
                  <label className="text-xs font-medium text-[#94A3B8] uppercase tracking-wider">使用模型</label>
                  <div className="mt-1.5 flex items-center gap-2">
                    <div className="px-3 py-1.5 bg-[#EFF6FF] text-[#1E40AF] text-sm font-medium rounded-lg">{selectedAgent.model}</div>
                  </div>
                </div>

                <div>
                  <label className="text-xs font-medium text-[#94A3B8] uppercase tracking-wider">技能配置（{selectedAgent.skills}个）</label>
                  <div className="mt-2 space-y-2">
                    {skillList.slice(0, selectedAgent.skills).map((skill) => (
                      <div key={skill.name} className="flex items-center justify-between p-3 bg-[#FAFAFA] rounded-xl border border-[#E2E8F0]">
                        <div>
                ...
                <div className="pt-4 space-y-3">
                  <button className="w-full h-11 flex items-center justify-center gap-2 bg-[#1E40AF] hover:bg-[#2563EB] text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-sm hover:shadow-md">
                    <Settings className="w-4 h-4" />
                    编辑配置
                  </button>
                  <button onClick={() => setSelectedAgent(null)} className="w-full h-11 flex items-center justify-center gap-2 text-sm font-medium text-[#64748B] border border-[#E2E8F0] rounded-xl hover:bg-[#F1F5F9] hover:border-[#CBD5E1] transition-all duration-200">
                    关闭
                  </button>
                </div>
              </div>
          ...
          >
            ...
            >
              <div className="sticky top-0 bg-white border-b border-[#E2E8F0] px-6 py-4 flex items-center justify-between z-10 rounded-t-2xl">
                ...
                <button onClick={() => setShowCreatePanel(false)} className="p-2 text-[#94A3B8] hover:text-[#0F172A] hover:bg-[#F1F5F9] rounded-lg transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="p-6 space-y-6">
                <div>
                  ...
                  <input type="text" placeholder="例如：内容创作助手" className="w-full h-12 px-4 bg-[#FAFAFA] border border-[#E2E8F0] rounded-xl text-sm text-[#0F172A] placeholder:text-[#94A3B8] focus:outline-none focus:border-[#3B82F6] focus:ring-2 focus:ring-[#3B82F6]/15 transition-all duration-200" />
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {["GPT-4o", "Claude-3.5", "GPT-4", "Claude-3"].map((model) => (
                      <button key={model} className="flex items-center gap-2 p-3 bg-[#FAFAFA] border border-[#E2E8F0] rounded-xl text-sm text-[#0F172A] hover:border-[#3B82F6] hover:bg-[#EFF6FF] transition-all duration-200">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {skillList.map((skill) => (
                      <label key={skill.name} className="flex items-center gap-3 p-3 bg-[#FAFAFA] border border-[#E2E8F0] rounded-xl cursor-pointer hover:border-[#CBD5E1] transition-all duration-200">
                ...
                <div>
                  ...
                  <textarea rows={3} placeholder="描述这个Agent的核心能力和使用场景..." className="w-full px-4 py-3 bg-[#FAFAFA] border border-[#E2E8F0] rounded-xl text-sm text-[#0F172A] placeholder:text-[#94A3B8] focus:outline-none focus:border-[#3B82F6] focus:ring-2 focus:ring-[#3B82F6]/15 transition-all duration-200 resize-none" />
                </div>

                <div className="pt-2 space-y-3">
                  <button onClick={() => setShowCreatePanel(false)} className="w-full h-12 flex items-center justify-center gap-2 bg-[#1E40AF] hover:bg-[#2563EB] text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-sm hover:shadow-md">
                    <Sparkles className="w-4 h-4" />
                    创建Agent
                  </button>
                  <button onClick={() => setShowCreatePanel(false)} className="w-full h-12 flex items-center justify-center gap-2 text-sm font-medium text-[#64748B] border border-[#E2E8F0] rounded-xl hover:bg-[#F1F5F9] hover:border-[#CBD5E1] transition-all duration-200">
                    取消
                  </button>
                </div>
              </div>
