...
import {
  Plus, FileText, Search, Clock, Type, CheckCircle,
} from "lucide-react";
...
export function KnowledgeSection({ isAdmin }: KnowledgeSectionProps) {
  ...
  const [editorMode, setEditorMode] = useState<"create" | "read" | "edit">("create");
  const [editingDocId, setEditingDocId] = useState<string | null>(null);

  const editingDocument = useMemo(
    () => documents.find((d) => d.id === editingDocId) || null,
  ...
  const handleSwitchMode = (mode: "read" | "edit") => {
    setEditorMode(mode);
  };

  const handleSave = (title: string, content: string) => {
    if (editorMode === "edit" && editingDocId) {
      updateDocument(editingDocId, title, content);
      // After save, switch back to read mode
      setEditorMode("read");
    } else {
  ...
  const handleDelete = () => {
    if (editingDocId && confirm("确定要删除这个文档吗？此操作不可撤销。")) {
      deleteDocument(editingDocId);
      setEditorOpen(false);
      setEditingDocId(null);
    }
  };
              ...
              return (
                ...
                >
                  <div className="flex flex-wrap gap-2 mb-3">
                    ...
                    <span className="inline-flex items-center px-2 py-0.5 bg-[#ECFDF5] text-[#059669] text-[10px] font-medium rounded-full">
